...
/*以后模块封装统一采用 对象方式注册；不要采用 exports.fangfa的方式，太碎片了*/
var _coreModule = {

    /*对js 基础类型进行方法扩展：在全局加载时调用*/
    exten: function() {
        ...
        if (!String.prototype.toLower && typeof String.prototype.toLower != 'function') {
            String.prototype.toLower = function() {
                return this.toLowerCase();
            };
        }

        if (!String.prototype.toUpper && typeof String.prototype.toUpper != 'function') {
        ...
        Array.prototype.remove = function(predicate) {
            ...
            if (_coreModule.isFunction(predicate)) {
        ...
        Array.prototype.sum = function(predicate) {
            ...
            for (var i = 0; i < this.length; i++) {
                ...
                results = results + predicate(this[i]);
            }
        ...
        //---***has forEach in es6 ,this is forEachElement ***------
        Array.prototype.forEachElement = function(action) {
            for (var i = 0; i < this.length; i++) {
                action(this[i]);
            }
        };
        ...
        //将数组转为逗号分割字符串
        Array.prototype.toString = function(spChar) {
            //var str = "";
            //t=t ? t : ',';
            //for (var i = 0; i < this.length; i++) str += this[i] + t;
            //return str.length>0?str.substring(0,str.length-1):"";

            return this.join(spChar);
        };
        ...
        //高性能去重：https://www.cnblogs.com/wisewrong/p/9642264.html
        Array.prototype.distinct = function() {
            ...
            for (let i of arr) {
                if (!obj[i]) {
                    result.push(i)
                    obj[i] = 1; //给对象注册Property，对象的Property是不能重复的，所以用这个做标记性能很高
                }
            }
        ...
        */
        Array.prototype.groupBy = function(key) {
            ...
            if (_coreModule.isNullOrUndefined(key)) {
        ...
        Array.prototype.lastOrDefault = function(predicate) {
            var idx= this.findLastIndex(predicate);
            if(idx>-1){
              return this[idx];
            }
    ...
    String: {
        ...
        "concat": function() {
            ...
            var sb_Result = new _coreModule.StringBuilder();
            if (arguments && arguments.length > 0) {
                var firstPara = arguments[0];
                //针对数组的连接
                if (firstPara && _coreModule.isArray(firstPara)) {
                    for (let item of firstPara) {
                        if (item) {
        ...
        /** 
         * 字符串占位符格式化
         * 支持C# 的String.Format {0} {1}的方式，最大支持占位符20个
         */
        "format": function(strTemplate, p1, p2, p3, p4, p5, p6, p7, p8, p9, p10, p11, p12, p13, p14, p15, p16, p17, p18, p19, p20) {
            return strTemplate.format(p1, p2, p3, p4, p5, p6, p7, p8, p9, p10, p11, p12, p13, p14, p15, p16, p17, p18, p19, p20)
        },
        "isNullOrEmpty": function(str) {
            return _coreModule.isNullOrEmpty(str);
        },
        "isNullOrWhiteSpace": function() {
            return _coreModule.isNullOrWhiteSpace(str);
        },
        /*
         * 使用指定的分隔符 连接数组成为字符串
         */
        "join": function(spChar, arrItems) {
            var sb_Result = new _coreModule.StringBuilder();
            if (arrItems && _coreModule.isArray(arrItems)) {
                for (var i = 0; i < arrItems.length; i++) {
                    if (i == 0) {
                        sb_Result.append(arrItems[i]);
                    } else {
                        sb_Result.append(spChar);
                        sb_Result.append(arrItems[i]);
                    }
 ...
 */
    StringBuilder: (function() {
        ...
        StringBuilder.prototype.appendFormat = function(value, p1, p2, p3, p4, p5, p6, p7, p8, p9, p10, p11, p12, p13, p14, p15, p16, p17, p18, p19, p20) {
            var valStr = value.format(p1, p2, p3, p4, p5, p6, p7, p8, p9, p10, p11, p12, p13, p14, p15, p16, p17, p18, p19, p20);
            this.data.push(valStr);
        };;
        StringBuilder.prototype.clear = function() {
            this.data = [];
        };;
        StringBuilder.prototype.remove = function(startIndex) {
            if (startIndex >= 0) {
                this.data = this.data.slice(0, startIndex).concat(this.data.slice(startIndex + 1, this.length));
            }
        };;
    ...
    */
    getQueryString: function(url, namename) {
    ...
    */
    formatUrl: function(url, paras) {
        if (_coreModule.isNullOrEmpty(url)) {
            return null;
        }
        ...
        for (var key in paras) {
            var value = paras[key];
            if (_coreModule.isNumber(value) || _coreModule.isString(value)) {
     ...
     * @returns {} 
    */
    compressJsonStr: function(inputString, ii) {
    ...
    isNumberString: function(str) {
        ...
        if (_coreModule.isNullOrEmpty(str)) {
            return false;
        }

        return _coreModule.isNumber(str);
    ...
    isNullOrEmpty: function(str) {
        ...
        if (!_coreModule.isNullOrUndefined(str)) {
     ...
     */
    GB2312ToUnicode: function(theString) {
     ...
     *字符串转化为 unicode
     */
    converCharToUnicode: function(theString) {
        var unicodeString = '';
     ...
     * unicode转化为字符串
     */
    convertUnicodeToChar: function(str) {
        str = eval("'" + str + "'");
        return str;
    },
...
/*封装基本的http同步请求访问方法--不支持异步请求，异步请求请使用axios*/
HttpRequest:{
    ...
    baseAction: function (opt) {
      ...
      var baseServerUrl = config.basePath;
      ...
      var options = {
              ...
              type: opt.type,
              cache: false,
              async: false,//note: this is sync  request ，do not removw it!!
              success: function (data) {
                  if (opt.callback != undefined) {
                    opt.callback(data, opt.callbackParas);
                  } 
              },
    ...
    /*GET 同步请求*/
    getSync: function (url, paras, callback, callbackParas) {
      if (!this.checkConfig(url)) {
        alert("参数有误，请检查您的参数");
        return;
      }
      //格式化 请求的url
      url = appCore.formatUrl(url, paras);
      var opt = this.createConfig();
      opt.fordwardUrl = url;
      opt.type = "GET";
      opt.callback = callback;
      opt.callbackParas = callbackParas;
  
      this.baseAction(opt);
    },
  
    /*POST同步请求*/
    postSync: function (url, data, callback, callbackParas) {
      if (!this.checkConfig(url) || data == undefined) {
        alert("参数有误，请检查您的参数");
        return;
      }
      var opt = this.createConfig();
    ...
    MessageBox: {
        ...
        "__currentLoadingInstance": null,
        "winMessage": null,
        "winLoading": null,
        "winMessageBox": null,
        "winNotification": null,

        "toast": function(msg, onCloseHandler, config) {
        ...
        "loading": function(msg) {
            ...
            var title = "加载中...";
            if (msg && !_coreModule.isNullOrEmpty(msg)) {
            ...
            //注册到当前对象
            //elementui:需要注意的是，以服务的方式调用的全屏 Loading 是单例的：若在前一个全屏 Loading 关闭前再次调用全屏 //Loading，并不会创建一个新的 Loading 实例，而是返回现有全屏 Loading 的实例：
            this.__currentLoadingInstance = this.winLoading.service(options);

        },

        "closeLoading": function() {
            //wx.hideLoading();
            if (this.__currentLoadingInstance) {
                this.__currentLoadingInstance.close()
            }
        },
        ...
        "show": function(msg, title) {
            ...
            if (_coreModule.isNullOrEmpty(title)) {
                title = "提示信息";
            }
            let options = {
                title: title,
            ...
            this.winMessageBox(options);

        },
        /*弹出确认框*/
        "confirm": function(content, confirmHandler, cancelHandler, beforeCloseHandler) {
        ...
        /*弹出成功提示框*/
        "success": function(msg, onCloseHandler) {
        ...
        "warn":function(msg,type){
            ...
            this.winNotification(options);
        },
        /*弹出错误提示*/
        "error": function(msg, okHandler, beforeCloseHandler) {

            if (_coreModule.isNullOrEmpty(msg)) {
                msg = "系统错误！";
            }
    ...
    Timer: (function() {
        ...
        Object.defineProperty(Timer.prototype, "Interval", {
            ...
            set: function(value) {
                ...
                if (this._jsInnerTimerObj) {
                    clearInterval(this._jsInnerTimerObj);
                }
            },
            enumerable: true,
            configurable: true
        ...
        Object.defineProperty(Timer.prototype, "Elapsed", {
            ...
            set: function(value) {
                ...
                this._jsInnerTimerObj = setInterval(this._handlerInvoker, this.Interval);
            },
            enumerable: true,
            configurable: true
        });
...
//注册全局
window.StringBuilder = _coreModule.StringBuilder;
export default _coreModule